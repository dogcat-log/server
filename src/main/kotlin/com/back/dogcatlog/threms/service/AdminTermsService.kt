...
class AdminTermsService(
     ...
     */
    @Transactional
    fun addNewTermsType(typeName: String, version: String, isRequired: Boolean = false): Terms {
        val existingType = termsTypeRepository.findByName(typeName)
        if (existingType != null) {
            throw CustomException(ErrorCode.TERMS_ALREADY_EXISTS)
        }

        // 새로운 TermsType 생성
        val newType = TermsTypeEntity(name = typeName)
        termsTypeRepository.save(newType)

        // 중복 버전 체크
        if (termsRepository.findByTermsTypeAndVersion(newType, version) != null) {
     ...
     */
    @Transactional
    fun addNewTermsVersion(requestBody : AdminAddNewTermsType) {
